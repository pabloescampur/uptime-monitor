...
const UptimeDashboard = () => {
  ...
  useEffect(() => {
    const analyzeUptime = async () => {
      ...
      for (const val of validators) {
        ...
        let signed_blocks_window = 0;
        try {
          const res = await axios.get(`${API_URL}/cosmos/slashing/v1beta1/signing_infos/${address}`);
          missed_blocks_counter = parseInt(res.data.val_signing_info.missed_blocks_counter);
          signed_blocks_window = parseInt(res.data.val_signing_info.index_offset);
        } catch (err) {
          console.error(`Error fetching slashing info for ${address}`, err);
        }
  ...
  const getMissedBlockColor = (missed) => {
    if (missed < 10) return 'text-green-600';
    if (missed <= 50) return 'text-yellow-600';
    return 'text-red-600';
  };
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-800 text-center sm:text-left">Lumera Validator Uptime (Last 30 Blocks)</h1>
          <div className="mt-4 sm:mt-0 flex items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-700">RPC Status:</span>
              <span className={`w-3 h-3 rounded-full ${rpcStatus ? 'bg-green-500' : 'bg-red-500'}`}></span>
              <span className="text-sm text-gray-600">{rpcStatus ? 'Active' : 'Inactive'}</span>
            </div>
            <div className="text-sm text-gray-700">Block Height: <span className="font-semibold">{latestHeight}</span></div>
          </div>
        </div>

        <div className="space-y-6">
          {Object.entries(uptimes).map(([address, data]) => {
            const missed = data.statuses.filter(s => s === 'Missed').length;
            return (
              <div key={address} className="bg-white rounded-xl shadow p-4">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                  ...
                  <div className="text-sm text-gray-500">
                    <span className="mr-4">VP: {((data.voting_power / 1e6).toFixed(2))}%</span>
                    <span className={`${getMissedBlockColor(missed)} font-semibold`}>Missed: {missed}</span>
                    <span className="ml-4 text-gray-600">Total Missed: {data.missed_blocks_counter}</span>
                  </div>
                </div>
